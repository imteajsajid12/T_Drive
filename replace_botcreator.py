...
with open('src/components/landing/LandingSections2.jsx', 'r') as f:
    content = f.read()

# The BotCreator component starts at `export const BotCreator = ({ dark }) => {`
# and ends right before `// ====== PRICING ======` (or similar). Let's find the boundaries.
start_marker = "export const BotCreator = ({ dark }) => {"
end_marker = "\n// ====== PRICING ======"

start_idx = content.find(start_marker)
end_idx = content.find(end_marker, start_idx)

new_comp = """export const BotCreator = ({ dark }) => {
...
if start_idx != -1 and end_idx != -1:
    new_content = content[:start_idx] + new_comp + content[end_idx:]
    with open('src/components/landing/LandingSections2.jsx', 'w') as f:
        f.write(new_content)
    print("Replaced BotCreator successfully.")
else:
